                    ...
                    if (user.feedAction[feedIndex].comments[i].new_comment) {
                      ...
                      var commentIndex = _.findIndex(
                        ...
                        function (o) {
                          return (
                            o.id ==
                            user.feedAction[feedIndex].comments[i].comment
                          );
                        }
                      );
...
async function visionAI(filename) {
  ...
  const [emotionresult] = await client.faceDetection(filename);
  if (emotionresult.faceAnnotations[0] != undefined) {
    Emotions.push("Joy: " + emotionresult.faceAnnotations[0].joyLikelihood);
    Emotions.push(
      "Sorrow: " + emotionresult.faceAnnotations[0].sorrowLikelihood
    );
    Emotions.push("Anger: " + emotionresult.faceAnnotations[0].angerLikelihood);
    Emotions.push(
      "Surprise: " + emotionresult.faceAnnotations[0].surpriseLikelihood
    );
  } else {
  ...
  labels.forEach((label) => {
    ...
    if (label.score > 0.8) {
      Label.push(
        label.description.charAt(0).toLowerCase() + label.description.slice(1)
      );
    }
  });
}
  ...
  try {
    ...
    classification.categories.forEach((category) => {
      ...
      if (category.confidence > 0.5) {
        ...
        res.forEach((element) => {
          Category.push(element.charAt(0).toLowerCase() + element.slice(1));
        });
      }
    });
  } catch (e) {
...
exports.newPost = (req, res) => {
  User.findById(req.user.id, async function (err, user) {
    ...
    if (req.file) {
      ...
      console.log("Emotion Likelihoods", Emotions);
...
exports.postUpdateFeedAction = (req, res, next) => {
  User.findById(req.user.id, (err, user) => {
    ...
    console.log(
      "##### FOUND post " + req.body.postID + " at index " + feedIndex
    );
...
exports.postUpdateProFeedAction = (req, res, next) => {
  User.findById(req.user.id, (err, user) => {
    ...
    //find the object from the right post in feed
    var feedIndex = _.findIndex(user.profile_feed, function (o) {
      return o.profile == req.body.postID;
    });
...
exports.postUpdateUserPostFeedAction = (req, res, next) => {
  User.findById(req.user.id, (err, user) => {
    ...
    //find the object from the right post in feed
    var feedIndex = _.findIndex(user.posts, function (o) {
    ...
    //Are we doing anything with a comment?
      ...
      var commentIndex = _.findIndex(
        ...
        function (o) {
          return o.commentID == req.body.commentID;
        }
      );
      ...
      //LIKE A COMMENT
        ...
        user.posts[feedIndex].comments[commentIndex].liked = user.posts[
          feedIndex
        ].comments[commentIndex].liked
          ? false
          : true;
        console.log(
          "^&^&^&^&^&User Post comments LIKE was: ",
      ...
      //FLAG A COMMENT
        ...
        user.posts[feedIndex].comments[commentIndex].flagged = user.posts[
          feedIndex
        ].comments[commentIndex].flagged
          ? false
          : true;
        console.log(
          "%^%^%^%^%^%User Post comments FLAG was: ",
