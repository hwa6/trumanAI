  ...
  let promise = new Promise((resolve, reject) => {
    ...
    ***/
    ...
    .then(function (result) {
      ...
      return new Promise((resolve, reject) => {
        ...
        CSVToJSON()
          .fromFile("./input/notifications.csv")
          .then(function (json_array) {
    ...
    .then(function (result) {
      //Convert the notification reply csv file to json, store in comment_list\
      return new Promise((resolve, reject) => {
        console.log("Reading notification reply list...");
        CSVToJSON()
          .fromFile("./input/actor_replies.csv")
          .then(function (json_array) {
    ...
    *************************/
    ...
    .then(function (result) {
      ...
      return new Promise((resolve, reject) => {
        async.each(
          notification_reply_list,
          function (new_notify, callback) {
            Actor.findOne({ username: new_notify.actor }, (err, act) => {
              ...
              if (act) {
                ...
                // add label
                //ADD NEW LABEL HERE HENRY
                notify.label = new_notify.label;
                notify.save(function (err) {
                  if (err) {
                    console.log(
                      "Something went wrong in Saving Notify Actor reply!!!"
                    );
                    // console.log(err);
                    callback(err);
                  }
      ...
      return new Promise((resolve, reject) => {
        async.each(
          ...
          function (new_notify, callback) {
            Actor.findOne({ username: new_notify.actor }, (err, act) => {
              if (err) {
                console.log("createNotificationInstances error");
                console.log(err);
                return;
              }
  ...
  *************************/
    ...
    .then(function (result) {
      ...
      return new Promise((resolve, reject) => {
        async.eachSeries(
          comment_list,
          function (new_replies, callback) {
            Actor.findOne({ username: new_replies.actor }, (err, act) => {
              if (act) {
                Script.findOne(
                  { post_id: new_replies.reply },
                  function (err, pr) {
                    if (pr) {
                      ...
                      pr.save(function (err) {
                        if (err) {
                          console.log(
                            "@@@@@@@@@@@@@@@@Something went wrong in Saving COMMENT!!!"
                          );
